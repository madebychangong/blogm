#!/usr/bin/env python3
"""
Gemini API를 사용한 자연스러운 블로그 원고 재구성
- 어색한 기계적 치환을 완전히 다시 쓰기
- 금칙어 회피
- 사람이 쓴 것처럼 자연스럽게
"""

import os
import google.generativeai as genai
from typing import Optional


class AIRewriter:
    """Gemini API를 사용한 원고 완전 재구성"""

    def __init__(self, api_key: Optional[str] = None):
        """
        초기화

        Args:
            api_key: Gemini API 키 (없으면 환경변수 GEMINI_API_KEY 사용)
        """
        self.api_key = api_key or os.getenv('GEMINI_API_KEY')

        if not self.api_key:
            raise ValueError(
                "Gemini API 키가 필요합니다. "
                "환경변수 GEMINI_API_KEY를 설정하거나 api_key 파라미터를 전달하세요."
            )

        # Gemini 설정
        genai.configure(api_key=self.api_key)
        # Gemini 2.5 Pro 모델 사용 (사용자 확인)
        self.model = genai.GenerativeModel('gemini-2.5-pro')

    def create_prompt(self, text: str, keyword: str) -> str:
        """재구성 프롬프트 생성 - 완전히 다시 쓰기!"""

        # 금칙어 리스트
        forbidden_words = [
            "네요", "가격", "광고", "구매", "병원", "진단", "효과", "약효",
            "상담", "시술", "의사", "환자", "판매", "투자", "후회",
            "보험", "재발", "대출", "비용", "의문", "의심",
            "산부인과", "부작용", "홍보성", "의구심", "증상", "증.상"
        ]

        prompt = f"""당신은 블로그 글을 자연스럽게 다시 쓰는 전문 작가입니다.

⚠️ **매우 중요:**
아래 입력된 원고는 **기계적으로 단어만 치환해서 엉망입니다.**
절대 그대로 쓰지 말고, **의미만 파악해서 완전히 새로 작성하세요!**

# 입력 (기계 치환으로 어색함)
키워드: {keyword}

어색한 원고:
{text}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

# 🎯 당신의 임무: 위 원고를 **완전히 새로 작성**

## ❌ 절대 이렇게 쓰지 마세요!

입력 원고의 어색한 표현을 **절대 그대로 복사하지 마세요:**

**BAD 예시 (기계적 치환 - 절대 금지!):**
```
❌ "이거 그런 모습이 시작된 지 6개월"
   → "그런 모습"이 뭔가요? 완전 어색함!

❌ "딱히 큰 개선는 못 봤어요"
   → "개선는"? 조사 오류!

❌ "개선가 있을지 못 믿겠고도 들고"
   → "개선가", "못 믿겠고도"? 말이 안 됨!

❌ "클리닉에서 호르몬 치료 안내도 받아봤는데"
   → 왜 "안내"? 부자연스러움!

❌ "부.작용이 무서워서"
   → 점(.) 왜 들어가 있음?

❌ "경비이 부담돼서"
   → "경비이"? 조사 틀림!

❌ "그런 모습 완화에 좋은"
   → "그런 모습 완화"? 이상함!
```

**→ 이런 표현들을 발견하면 문장 전체를 완전히 다시 쓰세요!**

## ✅ 올바른 접근: 의미만 보고 완전히 새로 작성

**GOOD 예시 (자연스럽게 다시 쓰기):**
```
입력: "이거 그런 모습이 시작된 지 6개월"
출력: "이런 증세가 나타난 지 6개월" ✓
출력: "이런 느낌이 시작된 지 6개월" ✓

입력: "딱히 큰 개선는 못 봤어요"
출력: "딱히 큰 도움은 못 받았어요" ✓
출력: "별로 나아지지 않았어요" ✓

입력: "건강기능식품이 정말 개선가 있을지 못 믿겠고도 들고"
출력: "건강기능식품이 정말 도움이 될지 믿기 어렵기도 하고" ✓
출력: "건강기능식품이 효과가 있을까 의심스럽기도 하고" ✓

입력: "클리닉에서 호르몬 치료 안내도 받아봤는데"
출력: "병원에서 호르몬 치료 상담받아봤는데" ✓

입력: "부.작용이 무서워서"
출력: "안 좋은 반응이 무서워서" ✓
출력: "부작용이 걱정되어서" ✓

입력: "경비이 부담돼서"
출력: "가격이 부담되어서" ✓
출력: "돈이 많이 들어서" ✓

입력: "그런 모습 완화에 좋은"
출력: "증세 완화에 좋은" ✓
출력: "개선에 도움되는" ✓
```

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

# 📋 작성 규칙

## 1. 금칙어 절대 사용 금지
❌ **절대 쓰면 안 되는 단어:** {', '.join(forbidden_words)}

이 단어들이 원고에 있으면 반드시 바꾸세요:
- "증상/증.상" → "증세", "이런 느낌", "몸 상태"
- "효과/약효" → "도움", "개선", "나아짐"
- "부작용/부.작용" → "안 좋은 반응", "걱정되는 점"
- "비용/경비" → "가격", "금액", "돈"
- "의구심/의심" → "믿기 어려워", "확신이 안 서"
- "산부인과" → "병원", "여성 전문 병원"
- "홍보성" → "광고 같은"

## 2. 조사 오류 절대 금지
**받침 규칙 반드시 지키기:**
- 받침 있음 (금액, 도움, 병원): ~이, ~은, ~을
- 받침 없음 (나, 개선, 거): ~가, ~는, ~를

❌ 잘못된 예: "개선는", "개선가", "경비이", "궁금하다도"
✅ 올바른 예: "개선은", "개선이", "금액이", "궁금한데도"

## 3. 점(.) 표현 제거
"부.작용", "증.상" 같은 표현 발견 시 완전히 자연스럽게 바꾸기

## 4. 문맥에 맞게 다시 쓰기
**핵심:** 단어만 바꾸지 말고 문장 전체를 다시 구성

예시:
- "그런 모습이 시작된" → 문맥 보고 "증세가 나타난" / "느낌이 시작된"
- "큰 개선는" → "큰 도움은" / "많이 나아지지"
- "안내도 받아봤는데" → "상담받아봤는데" / "얘기 들어봤는데"

## 5. 자연스러운 블로그 말투
✅ **구어체:**
- ~더라고요, ~거든요, ~던데요, ~잖아요
- "솔직히", "진짜", "정말", "근데", "그래서"

✅ **다양한 종결어미:**
- ~어요 (30%) / ~더라고요 (20%) / ~거든요 (15%)
- ~죠 (10%) / ~던데요 (10%) / ~습니다 (5%)
- "~요"만 반복 금지!

✅ **경험담처럼:**
- "저는 ~인데", "제 경우엔"
- "힘들어서", "고민돼서", "답답해서"

## 6. 키워드 유지
- 키워드 "{keyword}"는 정확히 2-3회만
- 키워드 뒤 조사 붙이지 않기
- 나머지는 "이거", "이런 거" 등으로

## 7. 원본 의미 유지
- 핵심 내용과 경험담은 그대로
- 길이 비슷하게 (±100자)
- 단락 구조 유지

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

# 🎯 최종 체크 (출력 전 반드시 확인!)

1. ✅ 금칙어 사용 안 함 ({', '.join(forbidden_words[:10])}... 등)
2. ✅ 조사 오류 없음 ("~이/가", "~은/는", "~을/를" 정확)
3. ✅ 어색한 표현 완전히 다시 씀 ("그런 모습" 같은 거 없음)
4. ✅ 점(.) 표현 제거됨
5. ✅ 사람이 쓴 것처럼 자연스러움
6. ✅ 키워드 2-3회

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

# 출력
재구성된 원고만 출력하세요. 설명이나 주석 없이 원고만.
"""
        return prompt

    def rewrite(self, text: str, keyword: str) -> str:
        """
        원고를 완전히 새로 작성

        Args:
            text: 기계 치환된 어색한 원고
            keyword: 키워드

        Returns:
            자연스럽게 다시 쓴 원고
        """
        try:
            prompt = self.create_prompt(text, keyword)
            response = self.model.generate_content(prompt)

            if response.text:
                return response.text.strip()
            else:
                print("⚠️ AI 재구성 실패: 응답 없음")
                return text

        except Exception as e:
            print(f"⚠️ AI 재구성 오류: {e}")
            return text


def test_rewriter():
    """테스트"""

    # API 키 확인
    api_key = os.getenv('GEMINI_API_KEY')
    if not api_key:
        print("❌ GEMINI_API_KEY 환경변수를 설정해주세요.")
        print("   export GEMINI_API_KEY='your-api-key'")
        return

    rewriter = AIRewriter()

    # 테스트: 어색한 기계 치환 원고
    test_text = """갱년기홍조 때문에 진짜 일상생활이 힘들어서 글 올려봅니다.
이거 그런 모습이 시작된 지 벌써 6개월이 넘었는데,
처음엔 그냥 피로 때문이라고 생각했어요.

클리닉에서 호르몬 치료 안내도 받아봤는데,
암 위험 같은 부.작용이 무서워서 선뜻 시작을 못하겠어요.
석류즙이나 칡즙이 갱년기에 좋다고 해서 꾸준히 먹어봤지만,
딱히 큰 개선는 못 봤어요.
비싼 한약도 먹어봤는데 금액이 부담돼서 중단했고요.

건강기능식품이 정말 개선가 있을지 못 믿겠고도 들고,
인터넷엔 소개성 후기들이 많아서 뭘 믿어야 할지 모르겠더라고요.

갱년기홍조 말고도 갱년기 그런 모습 완화에 좋은 다른 방법이나,
제가 모르는 더 나은 제품들이 있다면 추천해주시면 감사하겠습니다."""

    keyword = "갱년기홍조"

    print("=" * 80)
    print("AI 완전 재구성 테스트")
    print("=" * 80)
    print(f"\n키워드: {keyword}")
    print(f"\n[입력] 어색한 기계 치환 원고:")
    print(test_text)
    print("\n어색한 표현:")
    print("  - '그런 모습이 시작된' ❌")
    print("  - '개선는 못 봤어요' ❌")
    print("  - '개선가 있을지' ❌")
    print("  - '못 믿겠고도 들고' ❌")
    print("  - '부.작용' ❌")
    print("  - '그런 모습 완화에' ❌")

    print(f"\n🤖 AI가 완전히 다시 쓰는 중...")
    result = rewriter.rewrite(test_text, keyword)

    print(f"\n[출력] 자연스럽게 다시 쓴 원고:")
    print("=" * 80)
    print(result)
    print("=" * 80)

    # 검증
    print(f"\n✅ 검증:")

    # 금칙어 확인
    forbidden_check = ["네요", "효과", "약효", "증상", "부작용", "의구심"]
    forbidden_found = False
    for word in forbidden_check:
        if word in result:
            print(f"  ❌ 금칙어 '{word}' 발견됨!")
            forbidden_found = True

    if not forbidden_found:
        print(f"  ✅ 금칙어 없음")

    # 어색한 표현 확인
    awkward_patterns = [
        "그런 모습", "개선는", "개선가", "못 믿겠고도",
        "부.작용", "증.상", "경비이"
    ]
    awkward_found = False
    for pattern in awkward_patterns:
        if pattern in result:
            print(f"  ❌ 어색한 표현 '{pattern}' 여전히 있음!")
            awkward_found = True

    if not awkward_found:
        print(f"  ✅ 어색한 표현 모두 수정됨")

    # 키워드 출현 확인
    count = result.count(keyword)
    print(f"  키워드 출현: {count}회 (목표: 2-3회)")


if __name__ == '__main__':
    test_rewriter()
